...
#======================================================================#
  ...
  follow=""
  lsargs="h"
  size="5000" # Default minimum filesize
  lines="500"   # Default maximum lines

  while getopts fHhbl:s:t:v opt 2> /dev/null
  do
     case "$opt" in
        f ) follow='-follow'  ;;
...
#======================================================================#
  ...
  include='-size +'"$size"'k -exec ls -lc'"$lsargs"' {} \;'
  ...
  if [ `wc -l < $outfile` -eq 0 ]; then
     echo "No files found in $1 > $size kilobytes in size."
  else
...
#              either the default minimum size (5000K) or the minimum  #
